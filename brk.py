...
for i in range(0, len(brk_csv) - 1):
    ...
    if not math.isnan(brk_csv['price'].iloc[i]):
        if round(lok_min / brk_csv['price'].iloc[i] * 100, 1) < 80:
            print(brk_csv.index[i], brk_csv['price'].iloc[i], lok_min_index, lok_min,
                  round(lok_min / brk_csv['price'].iloc[i] * 100, 1))
            logger.info(str((brk_csv.index[i], brk_csv['price'].iloc[i], lok_min_index, lok_min,
                             round(lok_min / brk_csv['price'].iloc[i] * 100, 1))))
